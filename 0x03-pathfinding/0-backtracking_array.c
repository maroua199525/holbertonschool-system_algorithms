...
/**
 * add - creates a point and adds to a queue
 * @queue: queue
 * @x: X coordinate
 * @y: Y coordinate
 * Return: 1 if success, 0 if failed
 */
int add(queue_t **queue, int x, int y)
{
	point_t *p;
...
/**
 * bck_helper - backtracking helper function to find
...
*/
int bck_helper(queue_t **queue, int *visited, char **map, int rows,
			int cols, int x, int y, point_t const *target)
{
	...
	if (x == target->x && y == target->y)
		return (add(queue, x, y));
	*(visited + y * cols + x) = 1;

	for (i = 0; i < 4; ++i)
		if (bck_helper(queue, visited, map, rows, cols,
			    ...
			    point_t const *start, point_t const *target)
{
	...
	ret = bck_helper(&queue, visited, map, rows, cols,
